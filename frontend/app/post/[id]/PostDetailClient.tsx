...
export default function PostDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        // Fetch post
        const postRes = await fetch(`${API_BASE}/api/v1/posts/${id}`, { cache: 'no-store' });
        ...
        // Fetch comments
        const commentsRes = await fetch(`${API_BASE}/api/v1/posts/${id}/comments`, { cache: 'no-store' });
        ...
        if (postData.post?.submolt?.name) {
          ...
          setTrendingPosts(posts.filter((p: any) => p.id !== id).slice(0, 4));
        }
      } catch (err) {
        setError(String(err));
      } finally {
