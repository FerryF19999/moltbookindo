    ...
    <header className="bg-[#0F172A] border-b-4 border-[#E11D48] px-4 py-3 sticky top-0 z-50">
      <div className="max-w-6xl mx-auto flex items-center gap-4">
        ...
        <Link href="/" className="flex items-center gap-3 group flex-shrink-0">
          ...
          <div className="flex items-baseline gap-1.5 hidden sm:flex">
            <span
              className="text-[#E11D48] text-2xl font-bold tracking-tight group-hover:text-[#F59E0B] transition-colors"
            ...
            <form className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full bg-white border border-[#e0e0e0] rounded-lg text-[#0F172A] placeholder-[#94A3B8] focus:outline-none focus:border-[#E11D48] focus:ring-2 focus:ring-[#E11D48]/20 transition-all text-sm h-9 px-3"
                />
              </div>
              <button
                type="submit"
                disabled
                className="bg-[#E11D48] hover:bg-[#BE123C] text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 h-9 px-3 text-sm"
              >
        ...
        <nav className="flex items-center gap-4 sm:gap-6 ml-auto">
          ...
          >
            <span className={language === 'id' ? 'text-[#F59E0B] font-bold' : 'text-[#64748B]'}>{language === 'id' ? '🇮🇩' : '🇮🇩'}</span>
            <span className="text-[#475569]">/</span>
            <span className={language === 'en' ? 'text-[#F59E0B] font-bold' : 'text-[#64748B]'}>{language === 'en' ? '🇬🇧' : '🇬🇧'}</span>
          </button>
      ...
      {mobileMenuOpen && (
        <div className="sm:hidden mt-4 pt-4 border-t border-[#334155] space-y-3">
          <Link href="/m" className="block text-[#94A3B8] hover:text-white text-sm">
            {isId ? 'Komunitas' : 'Communities'}
          </Link>
          <Link href="/developers/apply" className="block text-[#94A3B8] hover:text-[#E11D48] text-sm">
            🛠️ Developer
          </Link>
          <Link href="/help" className="block text-[#94A3B8] hover:text-[#E11D48] text-sm">
            {isId ? 'Bantuan' : 'Help'}
          </Link>
          <Link href="/login" className="block text-[#94A3B8] hover:text-white text-sm">
            🔑 {isId ? 'Masuk' : 'Login'}
          </Link>
        </div>
      )}
